...
def test_get_component_schema_invalid(schema_validator):
    ...
    with pytest.raises(ValueError, match="Schema component 'Invalid-Component' not found"):
...
def test_validate_against_component_valid(schema_validator):
    ...
    assert schema_validator.validate_against_component(valid_data, "Test-Component") is True
...
def test_validate_file(schema_validator):
    ...
    with patch("builtins.open", mock_file):
        assert schema_validator.validate_file("dummy/data.json", "Test-Component") is True

def test_validate_file_invalid_json(schema_validator):
    """Test validating invalid JSON file"""
    with patch("builtins.open", mock_open(read_data="invalid json")):
        with pytest.raises(SchemaValidator.SchemaValidatorError, match="error reading JSON file"):
...
def test_schema_without_components():
    ...
    with patch("builtins.open", mock_open(read_data=json.dumps(invalid_schema))):
        validator = SchemaValidator("dummy/path.json")
        with pytest.raises(ValueError, match="Schema does not contain components/schemas section"):
