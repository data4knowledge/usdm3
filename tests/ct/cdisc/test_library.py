...
@patch("usdm3.ct.cdisc.library.Config")
@patch("usdm3.ct.cdisc.library.Missing")
def test_initialization_with_invalid_type(mock_missing_cls, mock_config_cls):
    """Test Library initialization with invalid type defaults to USDM"""
    library = Library("/test/path", "INVALID")
    assert library._type == "usdm"


@patch("usdm3.ct.cdisc.library.Config")
@patch("usdm3.ct.cdisc.library.Missing")
def test_initialization_with_all_type(mock_missing_cls, mock_config_cls):
    """Test Library initialization with ALL type"""
    library = Library("/test/path", "ALL")
    assert library._type == "all"


@patch("usdm3.ct.cdisc.library.Config")
@patch("usdm3.ct.cdisc.library.Missing")
def test_usdm_method(mock_missing_cls, mock_config_cls):
    """Test _usdm method"""
...
def test_load_all_ct_path(mock_cache_cls, mock_api_cls, mock_missing_cls, mock_config_cls):
    ...
    mock_api.refresh.assert_called_once()
    mock_api.all_code_lists.assert_called_once()
    assert mock_api.package_code_list.call_count == 2  # Called for each code list
    mock_cache.save.assert_called_once()
