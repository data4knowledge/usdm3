...
class DataStore:
    ...
    def _add_klass_instance(self, data, parent, path, instance_index) -> None:
        # print(f"ADD KLASS INSTANCE: {data['instanceType'] if 'instanceType' in data else 'Wrapper'}, {path}, {instance_index}")
        klass = data["instanceType"] if "instanceType" in data else "Wrapper"
        path = self._update_path(path, data, instance_index)
        if klass not in self._klasses:
            self._klasses[klass] = {}
        self._klasses[klass][data["id"]] = data
        self._ids[data["id"]] = data
        if parent:
            self._parent[data["id"]] = parent
        self._path[data["id"]] = path
        print(f"PATH: {path}")
        return path
